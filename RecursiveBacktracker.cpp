...
std::vector<int> RecursiveBacktracker::getUnvisitedNeighbors(int row, int col) {
...
bool RecursiveBacktracker::step() {
    ...
    std::vector<int> neighbors = getUnvisitedNeighbors(currentRow, currentCol);
    if (!neighbors.empty()) {
        // Choose a random neighbor.
        int dir = neighbors[rand() % neighbors.size()];
        cellStack.push(QPoint(currentRow, currentCol));
